...
// INFERENCE PHASE
void
run_inference(operator_node **node, TractValue **input_values)
{
    struct timeval t1_run, t2_run;
    double elapsed_time;
...
double
execute_tree(operator_node *node, TractValue **input_values, double elapsed_time, char **visited_nodes, int *visited_count, FILE *fd)
{
    if (!node) {
        return elapsed_time;
    }
    ...
    for (int i = 0; i < node->num_children; i++) {
        elapsed_time = execute_tree(node->children[i], input_values, elapsed_time, visited_nodes, visited_count, fd);
    }
    return elapsed_time;
}
...
inference_no_aes(float **images, int num_images, uint8_t *tokenizer, int tokenizer_size, model *m)
{
...
#endif
        if (!fd) {
            fprintf(stderr, "Error opening inference_time_cpu!\n");
            return NULL;
        }
        if (fprintf(fd, "Inference time: %f ms\n", elapsed_time) < 0) {
            fprintf(stderr, "Error writing to file inference_time_cpu\n");
            fclose(fd);
            return NULL;
        }
        fclose(fd);
    ...
    gettimeofday(&t1_inf, NULL);
    double sum = execute_tree(m->head, input_values, 0.0, visited_nodes, &visited_count, fd);
    gettimeofday(&t2_inf, NULL);
